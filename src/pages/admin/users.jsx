...
export const Users = () => {
  ...
  const deleteUser = async (email) => {
    const res = await axios.post("http://localhost/ecommerce%20project/admin/DeleteUser.php", { email });
    console.log(res)
  ...
  return (
    <div className={`pl-64 pt-20 bg-gray-200`}>
      <div className='text-center text-5xl mr-32 mb-12'>Comptes</div>
      ...
      <div className='mr-20 rounded-lg overflow-hidden bg-gray-50 space-y-4'>
        <div className='w-full grid grid-cols-6 px-10 h-12 text-xl font-mono text-white bg-black pt-2 rounded-xl'>
          <div>Photo</div>
          <div>Nom</div>
          <div>Mot de passe</div>
          <div>E-mail</div>
          <div>Date d'adhésion</div>
          <div className='w-56 -mr-4'>Supprimer</div>
        </div>
        {isLoading ? (
          <div role="status" className='flex justify-center h-72 items-center'>
            <svg aria-hidden="true" className="w-20 h-20 text-center text-gray-200 animate-spin dark:text-gray-600 fill-gray-300" viewBox="0 0 100 101" fill="none">
      ...
      {showConfirmation && (
        <div className=''>
          ...
          <div className='fixed top-1/3 bg-gray-100 text-center h-44 w-120 rounded-xl shadow-xl p-4' style={{ left: "40%" }}>
            ...
            <button className='absolute top-2 right-2 text-xl' onClick={() => setShowConfirmation(false)}>
              <TiDeleteOutline />
            </button>
          </div>
        </div>
